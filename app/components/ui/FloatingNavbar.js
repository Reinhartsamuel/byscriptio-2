...
export const FloatingNav = ({ navItems = defaultLinks, className }) => {
      ...
      >
        {navItems?.map((navItem, idx) => (
          ...
          >
            ...
            <span className="block text-[0.6rem]">{navItem.name}</span>
          </Link>
        ))}
        <button className="border text-[0.6rem] font-medium relative border-neutral-200 dark:border-white/[0.2] text-black dark:text-white px-4 py-2 rounded-full sm:text-sm">
          <span>Login</span>
