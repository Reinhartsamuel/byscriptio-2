...
// Helper to update DCA bot balance
async function updateDcaBotBalance(botId, profitUsd) {
  const botRef = adminDb.collection("dca_bots").doc(botId);
  const botSnap = await botRef.get();

  if (!botSnap.exists) return;

  const currentTradeAmount = parseFloat(botSnap.data().tradeAmount) || 0;
  const additionalAmount = parseFloat(profitUsd) || 0;

  await botRef.update({
    tradeAmount: currentTradeAmount + additionalAmount,
    updatedAt: new Date(),
  });
}
