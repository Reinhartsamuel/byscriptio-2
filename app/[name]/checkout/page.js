...
import { getCollectionFirebase, getSingleDocumentFirebase, updateDocumentFirebase } from '@/app/utils/firebaseApi';
...
export default function CheckoutPage() {
      ...
      if (Array.isArray(result) && result?.length > 0) {
        ...
        if (voucherData?.expiredAt?.seconds < moment().unix()) {
          ...
          if (resultSubscription?.length >= voucherData?.maxSlot) {
            ...
            let discount = voucherData?.type === 'DISCOUNT' ? voucherData?.amount : data?.price * voucherData?.amount / 100;
            let previousPrice = data?.price;
            let price = voucherData?.type === 'DISCOUNT' ? data?.price - parseInt(voucherData?.amount) : data?.price * ((100 - voucherData?.amount) / 100);
            updateDocumentFirebase('subscriptions', subscriptionId, {
              voucherCode,
              price,
              previousPrice,
  ...
  async function getData() {
    try {
      setLoading(true);
      const res = await getSingleDocumentFirebase('subscriptions', subscriptionId);
      setData(res);
        ...
        <div className="w-full md:w-1/2 bg-gray-800 rounded-lg p-4 shadow-md">
          ...
          <div className="bg-gray-700 rounded-lg p-4 mb-4">
            <div className="flex items-start justify-between">
              ...
              <div className="">
                <p className="text-base font-medium">IDR {data?.price?.toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-700 rounded-lg p-4 mb-4">
            ...
            <div className="flex items-center justify-between gap-2 mt-2">
              ...
              <button
                ...
                className={
                  cn("whitespace-nowrap px-4 py-2 text-white rounded", paymentLink?.length > 0 ? 'bg-gray-600 text-gray-400 cursor-not-allowed hover:bg-gray-600' : 'bg-indigo-600 hover:bg-indigo-700')
                }
              >
                Redeem
              </button>
          ...
          <div className="w-full bg-gray-800 rounded-lg p-4 shadow-md">
            ...
            <div className="w-full flex justify-between">
              ...
              <p className="text-base font-medium">IDR {data?.previousPrice ? data?.previousPrice?.toLocaleString() : data?.price?.toLocaleString()}</p>
            </div>
            {data?.discount &&
              <div className="w-full flex justify-between">
                <p className="text-sm text-gray-400">Discount</p>
                <p className="text-base font-medium">IDR {data?.discount?.toLocaleString()}</p>
              </div>
            }
            <div className="w-full flex justify-between">
          ...
          <button
            ...
            disabled={loading || paymentLink.length !== 0}
            className={cn("w-full px-4 py-4 flex items-center justify-center bg-indigo-600 text-white rounded hover:bg-indigo-700", (loading || paymentLink.length !== 0) && 'bg-gray-600 text-gray-400 cursor-not-allowed hover:bg-gray-600')}
          >
            {loading && <Spinner className='mr-2' />}
            <p>Pay IDR {data?.price?.toLocaleString()}</p>
          </button>
          <a className='mb-10' href={paymentLink} target='_blank' rel='noreferrer'>
            Payment page not showing? Manually click <span className='underline text-blue-400'>here</span> to proceed payment
          </a>
        </div>
